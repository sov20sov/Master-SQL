...
const DeveloperVoiceNote = () => {
  const [isPlaying, setIsPlaying] = useState(false);
  const [progress, setProgress] = useState(0);
  const [duration, setDuration] = useState(15);

  // ✅ مسارات صحيحة مع GitHub Pages base
  const audioUrl = useMemo(
    () => `${import.meta.env.BASE_URL}Media/massage-from-dev.mp3`,
    []
  );
  const imgUrl = useMemo(
    () => `${import.meta.env.BASE_URL}Media/man-with-computer-and-headset-svgrepo-com.svg`,
    []
  );

  // ✅ أنشئ Audio مرة واحدة
  ...
  const togglePlay = async () => {
    ...
    if (!isPlaying) {
      ...
      try {
        ...
        // إذا المتصفح منع التشغيل بدون تفاعل/صلاحيات
        setIsPlaying(false);
      }
    } else {
      setIsPlaying(false);
  ...
  return (
    <div className="max-w-xl mx-auto mb-20 px-4 animate-scale-in delay-200">
      <div className="glass-morphism rounded-full p-2 pr-6 flex items-center gap-4 transition-all duration-300 hover:scale-[1.02] hover:bg-white/10 group">

        {/* Play Button */}
        <button
          onClick={togglePlay}
          className="w-12 h-12 rounded-full bg-gradient-to-tr from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white flex items-center justify-center transition-all shadow-lg shadow-cyan-500/30 shrink-0 btn-interaction"
        >
          {isPlaying ? (
        ...
        <div className="flex-1 flex flex-col justify-center gap-1.5 min-w-0">
          <div className="flex justify-between items-center text-xs font-bold text-slate-500 dark:text-slate-400 group-hover:text-slate-800 dark:group-hover:text-slate-300 transition-colors">
            <span>رسالة من المطور</span>
            <span className="font-mono">
              {isPlaying
                ? `00:${Math.floor((progress / 100) * duration)
                    .toString()
                    .padStart(2, "0")}`
          ...
          {/* Waves */}
          <div className="flex items-center gap-[2px] h-6 w-full overflow-hidden" dir="ltr">
            {[...Array(45)].map((_, i) => (
              <div
                key={i}
